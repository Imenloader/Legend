...
window.SKILLS = {
    ...
    techniques: {
        ...
        // --- الأوراد والأذكار البدنية الفعالة (Spiritual Spells - Active) ---
        'badr_blessing': { id: 'badr_blessing', name: 'عزيمة النور والصلابة الصافية', desc: 'تركيز بدني صافي من ديوانك وقواتك بيرجع 30% من صحتك القصوى.', mpCost: 30, heal: 0.3, type: 'magic', reqLvl: 3 },
        ...
        'heaven_seal': { id: 'heaven_seal', name: 'ختم أقاليم الصحراء العالي الحافظ', desc: 'طلسم أسطوري: بيشل هالة الأعداء ويقلل هجومهم ودفاعهم بـ 40%.', mpCost: 75, debuff: { atk: -0.4, def: -0.4, duration: 5 }, type: 'magic', reqLvl: 30 },

        // --- القدرات البدنية الكامنة (Passive Techniques) ---
        'spirit_surge': { id: 'spirit_surge', name: 'فيض مانا القلعة وديوان', desc: 'قدرة كامنة: +10% سرعة استرجاع مانا تأمل الروح.', passive: true, stat: 'mpRegen', bonus: 0.1, reqLvl: 10 },
        ...
        'unending_dao': { id: 'unending_dao', name: 'فلسفة البقاء الأزلي وسر النور', desc: 'قدرة كامنة: +25% سرعة كسب النور البدني، و +10% لكل إحصائيات الجسد.', passive: true, stat: 'allStats', bonus: 0.1, reqLvl: 40 },
        'eternal_breath': { id: 'eternal_breath', name: 'الأنفاس المباركة الممتدة', desc: 'قدرة كامنة: بترجع 5% من صحتك وجتركيزك وهمتك تلقائي مع كل دور في المعركة.', passive: true, stat: 'hpRegen', bonus: 0.05, reqLvl: 22 }
    },
        ...
        Object.values(this.techniques).forEach(skill => {
            if (state.player.lvl >= skill.reqLvl && !state.player.skills.includes(skill.id)) {
                ...
                if (typeof narrate === 'function') {
                    narrate(`<b>تم تعلم مهارة وفن جديد:</b> ${skill.name}!`, "ديوان الفتوة");
                }
            }
        });
    },
